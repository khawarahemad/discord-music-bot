"""
Discord Music Bot — Fixed & Advanced (No Reactions)

This is a cleaned, robust and advanced version of your music bot.
Key fixes & improvements:
 - Corrected queue/current handling so `!queue` shows Now Playing + upcoming tracks reliably
 - ytdl_search always appends discovered tracks to the GuildMusic.queue
 - start_playback / next_track split and race-safe with asyncio.Lock per guild
 - Control panel buttons safely edit queue/state under lock
 - Spotify support preserved (if spotipy credentials present)
 - Safer yt-dlp usage via asyncio.to_thread
 - Improved error handling and logging
 - Single clean keep-alive thread and bot.run

Requirements:
 - discord.py v2+ (`pip install -U discord.py`)
 - yt-dlp (`pip install -U yt-dlp`)
 - ffmpeg available in PATH
 - (optional) spotipy for Spotify links (`pip install spotipy`) and provide SPOTIFY_CLIENT_ID/SECRET
 - a .env file with DISCORD_TOKEN (and optionally SPOTIFY creds)

Usage:
 - Put this file on your machine and run with Python 3.8+
...
# --- Optional cookie fetch (if you maintain a public cookie file) ---
...
def fetch_cookies_to_tempfile(url: str) -> Optional[str]:
    try:
        ...
        tmp.write(content)
        tmp.close()
        logger.info(f"Downloaded cookies to {tmp.name}")
        return tmp.name
    except Exception as e:
        logger.warning(f"Failed to fetch cookies: {e}")
        return None

COOKIES_PATH = fetch_cookies_to_tempfile(COOKIES_URL) if os.getenv("USE_REMOTE_COOKIES", "false").lower() == "true" else os.getenv("YTDLP_COOKIES")
...
try:
    ...
    sp = None
    logger.info("Spotify support disabled (spotipy missing or credentials not set).")
        ...
        try:
            ...
            if gm.message:
                try:
                    ...
                    gm.message = await (ctx.send(embed=embed, view=view) if ctx else bot.get_channel(gm.last_channel_id).send(embed=embed, view=view))
            else:
                if ctx:
...
async def next_track(ctx: Optional[commands.Context], gm: GuildMusic, skip_current: bool = False):
    ...
    This function will pop the next item from gm.queue and call start_playback.
    """
    guild = bot.get_guild(gm.guild_id)
    vc = guild.voice_client if guild else None
    if not vc:
    ...
    async with gm.lock:
        if gm.looping and not skip_current and gm.current:
            # Replay the same track: do nothing to queue/current
            pass
        else:
            # Move to the next track if available
            if gm.queue:
                gm.current = gm.queue.popleft()
            else:
                # Nothing left in queue; stop and disconnect
                gm.current = None
                try:
                    await vc.disconnect()
                except Exception:
                    logger.exception("Failed to disconnect vc when queue empty")
                # Optionally update control message
                if gm.message:
                    try:
    ...
    # Start playback of the new gm.current
...
class ControlPanel(View):
    ...
    async def previous(self, interaction: discord.Interaction, _):
        ...
        async with self.gm.lock:
            # Put current back to front, then rotate the last item to front to simulate previous
            if self.gm.current:
...
async def cmd_play(ctx: commands.Context, *, query: str):
    ...
    async with ctx.typing():
        try:
            ...
            if len(tracks) == 1:
                t = tracks[0]
                embed = discord.Embed(title="Added to Queue", description=f"**{t.title}**\n\n[Open on YouTube]({t.webpage_url})", color=discord.Color.blue())
                if t.thumbnail:
            ...
            # Ensure playback begins if nothing is currently playing
...
async def cmd_queue(ctx: commands.Context):
    ...
    embeds = []
    if gm.current:
        embed = discord.Embed(title=f"▶️ Now Playing: {gm.current.title}", description=f"[Open on YouTube]({gm.current.webpage_url})", color=discord.Color.orange())
    ...
    for i, track in enumerate(queued):
        embed = discord.Embed(title=f"{i+1}. {track.title}", description=f"[Open on YouTube]({track.webpage_url})", color=discord.Color.blue())
        if track.thumbnail:
...
async def cmd_help(ctx: commands.Context):
    embed = discord.Embed(title="🎵 BLIND MUSIC Bot Help", description="Welcome to **BLIND MUSIC**! Here’s a list of my commands:", color=discord.Color.purple())
...
# Keep-alive (optional simple Flask) — run in background thread so bot.run() isn't blocked
def keep_alive():
    try:
